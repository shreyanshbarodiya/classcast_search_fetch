...
class QuestionGymAPIView(generics.ListAPIView): # DetailView CreateView FormView
    ...
    def get_queryset(self):
        ...
        for topic in topic_list.iterator():
            submissions_easy=student_topic_interaction.objects.get(student_id=student_id,difficulty=0, topic_id=topic.topic_id).values('num_corrects')
            submissions_medium=student_topic_interaction.objects.get(student_id=student_id,difficulty=1, topic_id=topic.topic_id).values('num_corrects')
            submissions_difficult=student_topic_interaction.objects.get(student_id=student_id,difficulty=2, topic_id=topic.topic_id).values('num_corrects')
